...
if [[ -z "${EXTENSIONS##*,swoole,*}" ]]; then
    ...
    if [[ "$?" = "1" ]]; then
        ...
        nstallExtensionFromTgz swoole-2.0.11
    fi
fi

if [[ -z "${EXTENSIONS##*,hprose,*}" ]]; then
    # 不支持php8+  https://github.com/hprose/hprose-php/issues/151
    echo "---------- Install hprose ----------"
...
if [[ -z "${EXTENSIONS##*,yasd,*}" ]]; then
    ...
    if [[ "$?" = "1" ]]; then
             #curl -SL "https://github.com/swoole/yasd/archive/refs/tags/v0.3.9.tar.gz" -o yasd-0.3.9.tar.gz \
             mkdir -p yasd \
